        ...
        for flag in flags:
            try:
                if flag.type != "individual":
                    if flag.content.lower() == submission["submission"].strip().lower():
                        return True, "Correct"
                    continue
                # result = compare(flag, submission["submission"].strip())
                # if (result == 0 and flag.user_id == _user_id):
                if(flag.content.lower() == submission["submission"].strip().lower() and flag.user_id == _user_id):
                    correct_flag = True
                # if result > 0:
