...
class FlagList(Resource):
    ...
    def post(self):
        ...
        if req["type"]== "individual":
            ...
            for user in users:
                m = hashlib.sha1()
                _str = user.name + str(user.id) + str(req['challenge']) + "individual" + str(random.randint(0,1000))
                m.update(_str.encode('utf-8'))
                _content = m.hexdigest()
                dbflag = Flags.query.filter_by(challenge_id=req["challenge"],user_id=user.id,type="individual").count()

                if dbflag == 0:    
                    _flag = Flags(content=_content,data="",type="individual",challenge_id=req["challenge"],user_id=user.id)
...
class Flag(Resource):
    ...
    def get(self, flag_id):
        ...
        # user_name = Users.query.filter_by(id=flag.user_id).first_or_404().name
        ...
        return {"success": True, "data": response.data,"abc":True}
