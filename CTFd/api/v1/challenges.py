    ...
    def get(self, challenge_id):
        ...
        if authed():
            ...
            for f in chal.files:
                ...
                if f.user_id == user.id or f.user_id is None:
    ...
    def post(self):
        ...
        if not solves:
            ...
            if status:  # The challenge plugin says the input is right
                ...
                _flag= Flags.query.filter_by(challenge_id=challenge_id).first()

                if _flag.type == "online":
                    new_flag = _flag.content
                    condition = True
                    while condition:
                        
                        num = random.randint(20,36)
                        new_flag = ''.join(random.choices(string.ascii_letters + string.digits, k=num))
                        new_flag = "Flag{" + new_flag + "}"
                        F = Flags.query.filter_by(content=new_flag).all()
                        if F :
                            condition = True
                        else:
                            condition = False
                    ...
                    for location in locations:
                        if location["id"] == challenge_id:
                            change_flag(_flag.content,new_flag,location["path"])
                            if "execute" in location:
                                subprocess.call(location["execute"])
                    _flag.content = new_flag
                    db.session.commit()
...
locations = [
    ...
    {
     "id":21,
     "path":"/opt/CTFd/CTF_Challenges/i-hate-emacs/www/administration/index.php"
    },
    {
     "id":22,
     "path":"/opt/CTFd/CTF_Challenges/no-socket/challenge.py"
    },
    {
     "id":24,
     "path":"/opt/CTFd/CTF_Challenges/IAmBrute/challenge/db.json"
    },
    {
     "id":26,
     "path":"/opt/CTFd/CTF_Challenges/DockingStation/api/public/containers/7fbd7d717462b5510a0066a4e33884e6b73877e837379bf1c196679c7f504825/Export/home/flag_is_here/flag.txt",
     "execute":"/opt/CTFd/CTF_Challenges/DockingStation/api/public/containers/7fbd7d717462b5510a0066a4e33884e6b73877e837379bf1c196679c7f504825/change.sh"
    },
    {
    "id":27,
     "path":"/opt/CTFd/CTF_Challenges/forever-alone/thisistheflag.txt"
    }
...
def change_flag(_flag,new_flag,path):

    fin = open(path, "rt")
    data = fin.read()
    data = data.replace(_flag,new_flag)
...
# client.connect(ADDR)
# def send(msg1,msg2,msg3,msg4):
    
#     message1,send_length1 = get_msg(msg1)
#     message2,send_length2 = get_msg(msg2)
#     message3,send_length3 = get_msg(msg3)
#     message4,send_length4 = get_msg(msg4)


#     client.send(send_length1)
#     client.send(send_length2)
#     client.send(send_length3)
#     client.send(send_length4)
